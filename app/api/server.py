...
app = FastAPI(title="时察千机工业时序分析服务", version="0.2.0") if FastAPI else None
...
def _parse_config(payload: dict[str, Any] | None) -> AnalysisConfig:
    ...
    return AnalysisConfig(
        ...
        threshold=float(payload.get("threshold", settings.anomaly_threshold)),
...
if app:
    ...
    def model_compare(
        ...
        requested = payload.get("detectors", ["mad", "isolation_forest", "hybrid"])
        detectors = [str(item) for item in requested]
        allowed = {"mad", "isolation_forest", "hybrid"}
        if not detectors or any(detector not in allowed for detector in detectors):
            raise HTTPException(status_code=400, detail="detectors 只能包含 mad、isolation_forest、hybrid")
        ...
        try:
            ...
            for detector in detectors:
                ...
                result = analyze_file(matches[0], config=config, write_report=False)
